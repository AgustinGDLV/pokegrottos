...
static const u8 sText_CantThrowPokeBall_PartyFull[] = _("Cannot throw a ball!\nParty is full!\p");
...
static bool32 CannotUseBagBattleItem(u16 itemId)
{
    ...
    {
        // No PC, can't use ball because party is full.
        if (CalculatePlayerPartyCount() == 6)
        {
            failStr = sText_CantThrowPokeBall_PartyFull;
            cannotUse++;
        }
        switch (GetBallThrowableState())
        {
...
void ItemUseOutOfBattle_SuperEvolutionStone(u8 taskId)
{
    gItemUseCB = ItemUseCB_SuperEvolutionStone;
    SetUpItemUseCallback(taskId);
}
