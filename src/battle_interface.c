...
static void UpdateNickInHealthbox(u8 healthboxSpriteId, struct Pokemon *mon)
{
    ...
    default:
        ...
        windowTileData = AddTextPrinterAndCreateWindowOnHealthbox(gDisplayedStringBattle, 0, 3, HEALTHBOX_TRANSPARENT, &windowId);
        break;
    case MON_MALE:
...
static void UpdateStatusIconInHealthbox(u8 healthboxSpriteId)
{
    ...
    else
    {
        ...
        for (i = 0; i < 3; i++)
            CpuCopy32(statusGfxPtr + (battlerId * TILE_SIZE_4BPP), (void *)(OBJ_VRAM0 + (gSprites[healthboxSpriteId].oam.tileNum + tileNumAdder + i) * TILE_SIZE_4BPP), 32);
